...
#include "h264.hpp"
	...
	namespace H264 
	{
		...
		int Stream::nalUnitCount()
		{
			if(!Basic::isInit()) return -1;
		...
		void Stream::load(const char *filename)
		{
			...
			ret = fileLoad(filename, buf);
			if(ret < 0) return;

			m_pos = (uint8_t *) *buf;
			m_end = (uint8_t *) *buf + ret;
			cur = m_pos;
		}

		Parser & Stream::parser(Parser & p)
		{
			if(!Basic::isInit()) return p;
		...
		NalUnit & Parser::nextNalUnit(NalUnit & nu)
		{
			if(!isInit()) return nu;
		...
		AccessUnit & Parser::nextAccessUnit(AccessUnit & au)
		{
			if(!isInit()) return au;
		...
		{
			if(!Basic::isInit()) return false;
			if(startCode == nullptr) return false;

			return true;
		}

		int AccessUnit::naluCount()
		{
			if(!isInit()) return -1;
